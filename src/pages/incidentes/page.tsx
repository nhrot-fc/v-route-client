...
import { useIncidents, type IncidentFilterParams } from "@/hooks/use-incidents";
import { useVehicles } from "@/hooks/use-vehicles";
import { type IncidentDTO, IncidentTypeEnum } from "@/lib/api-client";
...
export default function IncidentesPage() {
  ...
  const { incidents, loading, error, refetch, totalPages, totalItems } =
    useIncidents(
      filter as IncidentFilterParams,
      {
        page: currentPage - 1,
        size: pageSize,
      }
  ...
  const mappedIncidents: IncidentDTO[] = incidents.map((incident) => ({
    ...incident,
  ...
  const columns = [
    {
      header: "ID",
      accessorKey: "id" as keyof IncidentDTO,
      className: "font-medium",
    },
    {
      header: "Vehículo",
      accessorKey: "vehicleId" as keyof IncidentDTO,
      cell: (incident: IncidentDTO) => (
    ...
    {
      header: "Tipo",
      accessorKey: "type" as keyof IncidentDTO,
      cell: (incident: IncidentDTO) => {
    ...
    {
      header: "Descripción",
      accessorKey: "description" as keyof IncidentDTO,
    },
    {
      header: "Fecha",
      accessorKey: "occurrenceTime" as keyof IncidentDTO,
      cell: (incident: IncidentDTO) => (
        <div className="flex items-center">
          <CalendarClock className="h-4 w-4 mr-1.5 text-gray-500" />
          {incident.occurrenceTime &&
            format(new Date(incident.occurrenceTime), "dd/MM/yyyy HH:mm", {
              locale: es,
            })}
        </div>
      ),
