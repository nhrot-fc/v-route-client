...
export function SimulationCanvas({ simulationState }: SimulationCanvasProps) {
  ...
  const drawIcon = useCallback(
    (ctx: CanvasRenderingContext2D, icon: HTMLImageElement, x: number, y: number, size: number) => {
      if (!icon) return;
  ...
  const getGlpColorLevel = useCallback((current: number, capacity: number): string => {
  ...
  const getOrderSizeCategory = useCallback(
    (volume: number): "small" | "medium" | "large" => {
  ...
  // Wrap drawSimulation in useCallback
  const drawSimulation = useCallback((state: SimulationStateDTO) => {
    ...
    if (state.pendingOrders) {
      state.pendingOrders.forEach((order) => {
        const isOverdue = order.deadlineTime
          ? new Date(order.deadlineTime) < new Date(state.currentTime || "")
          : false;
    ...
    if (state.vehicles) {
      state.vehicles.forEach((vehicle) => {
        ...
        if (!tooltip.show) {
          ...
          ctx.fillText(
            vehicleText,
            scaleX(x) + vehicleSize + 4 * (zoom / 15),
            scaleY(y),
          );
          ...
          ctx.fillRect(
            scaleX(x) - barWidth / 2,
            scaleY(y) + vehicleSize + barSpacing,
            barWidth,
            barHeight,
          );
          ...
          ctx.fillRect(
            scaleX(x) - barWidth / 2,
            scaleY(y) + vehicleSize + barSpacing,
            barWidth * glpPercentage,
            barHeight,
          );

          // Fuel bar
          const fuelPercentage =
            (vehicle.currentFuelGal || 0) / (vehicle.fuelCapacityGal || 1);
          const fuelColor =
            fuelPercentage <= 0.2
              ? "#ef4444"
              : fuelPercentage <= 0.4
          ...
          ctx.fillRect(
            scaleX(x) - barWidth / 2,
            scaleY(y) + vehicleSize + barSpacing + barHeight + 1,
            barWidth,
            barHeight,
          );
  ...
  // Use drawSimulation in effect and include it in dependencies
  useEffect(() => {
    if (simulationState && isImagesLoaded) {
      drawSimulation(simulationState);
    }
  }, [simulationState, isImagesLoaded, mousePosition, drawSimulation]);
