...
import { FileText, Upload, AlertCircle, CheckCircle2, CalendarIcon } from "lucide-react"
...
export function MaintenanceUploadForm() {
  ...
  const parseFile = async (fileContent: string): Promise<MaintenanceDTO[]> => {
    ...
    for (let i = 0; i < lines.length; i++) {
      try {
        ...
        if (parts.length < 2) {
          throw new Error(`Formato inválido en línea ${i + 1}. Esperado: vehicleId:assignedDate`)
        }
        ...
        if (!vehicleId || !vehicleIds.has(vehicleId)) {
          throw new Error(`El vehículo con ID ${vehicleId} no existe en la línea ${i + 1}`)
        }
        ...
        throw new Error(`Error en línea ${i + 1}: ${error instanceof Error ? error.message : 'Formato inválido'}`)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const maintenances = await parseFile(fileContent)
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Formato esperado: vehicleId:01d00h24m (donde 01d es el día, 00h es la hora y 24m son los minutos)
            </p>
          </div>
          ...
          {uploadStatus === "success" && (
            <Alert>
              ...
              <AlertDescription>Mantenimientos cargados correctamente</AlertDescription>
            </Alert>
          )}
          ...
          <div className="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-md text-sm">
            ...
            <p className="mt-2 text-xs">
              Donde T001 es el ID del vehículo y 01d00h24m indica el día 1 a las 00:24 horas.
              <br />El año y mes se especifican en los campos superiores.
            </p>
          </div>
        </div>
      </CardContent>
